...
struct HighlightedString {
    constexpr HighlightedString(std::string_view str, int from = -2, int to = -2)
        : m_from(from == -1 ? str.size() : from), m_to(to == -1 ? str.size() : to), m_og_str(str)
    {
        update_string_substr();
    }

    constexpr HighlightedString() : m_from(-2), m_to(-2), m_og_str(""), str("") {}
...
private:
    ...
    constexpr void update_string_substr() { str = m_og_str.substr(m_from, m_to - m_from); }
};
