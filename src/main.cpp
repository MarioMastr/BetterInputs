...
void BetterTextInputNode::setString(gd::string str) { setAndUpdateString(str); }
        ...
        if (pos != 0 && m_fields->m_string[pos - 1] == ' ') {
            ...
            if (m_fields->m_pos == -1) {
                ...
                this->m_cursor->setPositionX(
                    getCharNodePosInfo(cursorTextLabelInfo.numCharsFromStart - (m_fields->m_pos != 0), m_fields->m_pos == 0).position.x);
        }
    }
}
...
{
    if (!m_fields->m_highlighted.isHighlighting() && (m_fields->m_string.empty() || m_fields->m_pos == -1))
        return;

    if (m_fields->m_highlighted.isHighlighting() && !isShift) {
        updateBlinkLabelToCharForced(m_fields->m_highlighted.getToPos(false) == m_fields->m_string.length() ? -1
    ...
    if (isCtrl && isShift) {
        ...
        updateBlinkLabelToCharForced(BI::utils::findNextSeparator(m_fields->m_string, m_fields->m_pos));

        m_fields->m_is_adding_to_highlight = false;

        if (m_fields->m_highlighted.isHighlighting())
            highlightFromToPos(currentPos == m_fields->m_highlighted.getFromPos() ? m_fields->m_pos : m_fields->m_highlighted.getFromPos(),
                               currentPos == m_fields->m_highlighted.getFromPos() ? m_fields->m_highlighted.getToPos() : m_fields->m_pos);
        else
    ...
    if (isCtrl)
        return updateBlinkLabelToCharForced(BI::utils::findNextSeparator(m_fields->m_string, m_fields->m_pos));
...
{
    if (!m_fields->m_highlighted.isHighlighting() && (m_fields->m_string.empty() || m_fields->m_pos == 0))
        return;

    if (m_fields->m_highlighted.isHighlighting() && !isShift) {
        updateBlinkLabelToCharForced(m_fields->m_highlighted.getFromPos() == m_fields->m_string.length() ? -1 : m_fields->m_highlighted.getFromPos());
        clearHighlight();
        return;
    }
    ...
    if (isCtrl && isShift) {
        ...
        updateBlinkLabelToCharForced(BI::utils::findPreviousSeparator(m_fields->m_string, m_fields->m_pos));

        m_fields->m_is_adding_to_highlight = false;

        if (m_fields->m_highlighted.isHighlighting())
            highlightFromToPos(currentPos == m_fields->m_highlighted.getToPos() ? m_fields->m_pos : m_fields->m_highlighted.getToPos(),
                               currentPos == m_fields->m_highlighted.getToPos() ? m_fields->m_highlighted.getFromPos() : m_fields->m_pos);
        else
    ...
    if (isCtrl)
        return updateBlinkLabelToCharForced(BI::utils::findPreviousSeparator(m_fields->m_string, m_fields->m_pos));
...
{
    if (m_fields->m_string.empty() || m_fields->m_pos == 0)
        return;

    if (this->m_textArea) {
        const auto &cursorTextLabelInfo = getTextLabelInfoFromPos(m_fields->m_pos);

        if (cursorTextLabelInfo.numCharsFromLabelStart == 0)
            return;

        if (isShift)
            highlightFromToPos(m_fields->m_highlighted.isHighlighting()
                                   ? m_fields->m_highlighted.getToPos()
                                   : cursorTextLabelInfo.numCharsFromStart - cursorTextLabelInfo.numCharsFromLabelStart - 1,
                               m_fields->m_pos);
        else
            updateBlinkLabelToCharForced(cursorTextLabelInfo.numCharsFromStart - cursorTextLabelInfo.numCharsFromLabelStart - 1);
    }
    else {
        if (isShift)
            highlightFromToPos(0, m_fields->m_highlighted.isHighlighting() ? m_fields->m_highlighted.getToPos() : m_fields->m_pos);
        else
            updateBlinkLabelToCharForced(0);
    }
}
...
void BetterTextInputNode::onEndKey(bool isShift)
{
    ...
    if (m_fields->m_string.empty() || m_fields->m_pos == -1)
        return;
    ...
    if (this->m_textArea) {
        ...
        int targetPos =
            m_fields->m_pos + (std::string_view(cursorTextLabelInfo.label->getString()).length() - cursorTextLabelInfo.numCharsFromLabelStart) - 1;

        if (targetPos - 1 == cursorTextLabelInfo.numCharsFromLabelStart
            && this->m_textArea->m_label->getChildrenCount() >= cursorTextLabelInfo.line) {
            // get the end of the next label
            targetPos =
                targetPos
                + std::string_view(static_cast<CCLabelBMFont *>(this->m_textArea->m_label->getChildren()->objectAtIndex(cursorTextLabelInfo.line + 1))
                                       ->getString())
                      .length();

            if (isShift)
                highlightFromToPos(m_fields->m_highlighted.isHighlighting() ? m_fields->m_highlighted.getFromPos() : m_fields->m_pos, targetPos);
            else
        ...
        if (isShift)
            highlightFromToPos(m_fields->m_highlighted.isHighlighting() ? m_fields->m_highlighted.getFromPos() : m_fields->m_pos, -1);
        else
            updateBlinkLabelToCharForced(-1);
    }
}
...
// getters and setters
void BetterTextInputNode::useUpdateBlinkPos(bool toggle) { m_fields->m_use_update_blink_pos = toggle; }
...
void BetterTextInputNode::highlightFromToPos(int from, int to)
{
    ...
    if (this->m_placeholderLabel) {
        ...
        if (m_fields->m_string[to == -1 ? m_fields->m_string.length() - 1 : to] == ' ') {
            const auto &secondToLastCharNode = getCharNodePosInfo((to == -1 ? m_fields->m_string.length() - 1 : to) - 1, false);
        ...
        m_fields->m_highlights[0]->setContentWidth(std::abs(m_fields->m_highlights[0]->getPositionX() - targetXPos));

        m_fields->m_highlights[0]->setScaleY(3.f * this->m_placeholderLabel->getScale() + .4f);
        ...
        if (talSize != highlightsSize) {
            if (talSize > highlightsSize)
                while (m_fields->m_highlights.size() < talSize) appendHighlightNode();
            else
                while (m_fields->m_highlights.size() > talSize) removeLastHighlightNode();
        }
        ...
        // beware! very freaky code below
        {
            ...
            for (auto *label : textAreaLabels) {
                ...
                highlightSprite->setPositionY(this->convertToNodeSpace(this->m_textArea->m_label->convertToWorldSpace(label->getPosition())).y
                ...
                if (!hasHighlightedStart) {
                    if (targetFrom <= labelStrLen) {
                        const auto &textLabelInfo = getTextLabelInfoFromPos(
                            m_fields->m_highlighted.isHighlighting() ? m_fields->m_highlighted.getToPos(false) : m_fields->m_pos);
                        ...
                        if (targetTo > labelStrLen) {
                            ...
                            if (textLabelInfo.numCharsFromLabelStart == std::string_view(textLabelInfo.label->getString()).length() - 1)
                                highlightSprite->setContentWidth(
                                    std::abs(highlightSprite->getPositionX()
                                             - getCharNodePosInfoAtLine(textLabelInfo.numCharsFromLabelStart, textLabelInfo.line, false).position.x));
                            else
                                highlightSprite->setContentWidth(
                                    std::abs(highlightSprite->getPositionX() - getCharNodePosInfoAtLine(targetTo, line, true).position.x));
                    ...
                    if (targetTo > labelStrLen) {
                        ...
                        highlightSprite->setContentWidth(
                            std::abs(highlightSprite->getPositionX() - getCharNodePosInfoAtLine(targetTo, line, to != -1).position.x));
                        ...
                        highlightSprite->setVisible(
                            std::string_view(
                                static_cast<CCLabelBMFont *>(this->m_textArea->m_label->getChildren()->objectAtIndex(line - 1))->getString())
                                    .back()
                                != ' '
...
{
    if (!BI::geode::get<bool>("allow-any-character") && std::string_view(this->m_allowedChars).find(character) == std::string_view::npos)
        return;
    ...
    setAndUpdateString(BI::utils::insertCharAtIndex(m_fields->m_string, wasHighlighting ? m_fields->m_pos : pos, character));
...
{
    if (!m_fields->m_highlighted.isHighlighting() && (m_fields->m_string.empty() || (isDel && pos == -1) || (!isDel && pos == 0)))
        return;
...
CharNode BetterTextInputNode::getCharNodePosInfo(std::size_t pos, bool isLeftAnchored)
{
    ...
    if (this->m_placeholderLabel) {
        auto charNode = static_cast<CCNode *>(this->m_placeholderLabel->getChildren()->objectAtIndex(pos));

        CCPoint charNodeNodeSpacePos = this->convertToNodeSpace(this->m_placeholderLabel->convertToWorldSpace(charNode->getPosition()));

        const float offset = (charNode->getContentWidth() * this->m_placeholderLabel->getScaleX()) / 2;
        return { { isLeftAnchored ? (charNodeNodeSpacePos.x - offset) : (charNodeNodeSpacePos.x + offset), charNodeNodeSpacePos.y },
        ...
        CCPoint charNodeNodeSpacePos = this->convertToNodeSpace(targetLabel->convertToWorldSpace(charNode->getPosition()));
...
{
    auto targetLabel = static_cast<CCLabelBMFont *>(this->m_textArea->m_label->getChildren()->objectAtIndex(line));
...
InputNodeTextAreaInfo BetterTextInputNode::getTextLabelInfoFromPos(std::size_t pos)
{
    ...
    if (this->m_textArea) {
        ...
        return { targetLabel, line, pos, targetLabelPos - 1 };
    }

    return { this->m_placeholderLabel, 0, pos, pos };
}
...
void BetterTextInputNode::setAndUpdateString(const std::string &str)
{
    ...
    if (!BI::geode::get<bool>("allow-any-character"))
        std::erase_if(m_fields->m_string, [&](char c) -> bool { return std::string_view(this->m_allowedChars).find(c) == std::string_view::npos; });

    if (!BI::geode::get<bool>("bypass-length-check") && this->m_maxLabelLength != 0 && m_fields->m_string.length() >= this->m_maxLabelLength) {
...
{
    for (auto *highlight : m_fields->m_highlights) highlight->setVisible(false);
...
cocos2d::extension::CCScale9Sprite *BetterTextInputNode::appendHighlightNode()
{
    ...
    highlight->setPosition(this->m_placeholderLabel ? CCPoint{ this->m_cursor->getPosition() - 1.f } : CCPoint{ .0f, .0f });
...
const char *BetterCCTextFieldTTF::getString()
{
    if (g_selectedInput)
...
void AlertLayerFix::onUpdateTick(float)
{
    ...
    if (currentChildrenCount == 1 || currentChildrenCount == m_fields->m_previous_scene_children_count)
        return;

    if (!m_fields->m_outermost_input_parent) {
        CCLayer *outermostInputParent = static_cast<CCLayer *>(g_selectedInput->getParent());
        while (outermostInputParent->getParent() != this) outermostInputParent = static_cast<CCLayer *>(outermostInputParent->getParent());
    ...
    if (const int lastLayerTouchPrio = lastLayer->getTouchPriority();
        lastLayerTouchPrio != 0
        && lastLayerTouchPrio < CCTouchDispatcher::get()->findHandler(static_cast<CCTouchDelegate *>(g_selectedInput))->getPriority())
...
void BetterCCIMEDispatcher::dispatchDeleteBackward()
{
    if (g_selectedInput)
        return;
...
void BetterCCIMEDispatcher::dispatchDeleteForward()
{
    if (g_selectedInput)
        return;
...
void BetterCCIMEDispatcher::dispatchInsertText(const char *text, int len, cocos2d::enumKeyCodes keyCode)
{
    if (g_selectedInput)
...
void BetterCCEGLView::onGLFWKeyCallback(GLFWwindow *window, int key, int scancode, int action, int mods)
{
    if (!g_selectedInput)
    ...
    if (action != GLFW_RELEASE) {
        if (!BI::platform::keyDown(BI::PlatformKey::LEFT_CONTROL) && !BI::platform::keyDown(BI::PlatformKey::LEFT_SHIFT)) {
            switch (key) {
                case GLFW_KEY_ESCAPE: g_selectedInput->deselectInput(); break;

                case GLFW_KEY_BACKSPACE:
                case GLFW_KEY_DELETE: g_selectedInput->onDelete(false, key == GLFW_KEY_DELETE); break;
        ...
        switch (key) {
            case GLFW_KEY_RIGHT:
                g_selectedInput->onRightArrowKey(BI::platform::keyDown(BI::PlatformKey::LEFT_CONTROL),
                                                 BI::platform::keyDown(BI::PlatformKey::LEFT_SHIFT));
                break;

            case GLFW_KEY_LEFT:
                g_selectedInput->onLeftArrowKey(BI::platform::keyDown(BI::PlatformKey::LEFT_CONTROL),
                                                BI::platform::keyDown(BI::PlatformKey::LEFT_SHIFT));
                break;
    ...
    if (action == GLFW_PRESS && BI::platform::keyDown(BI::PlatformKey::LEFT_CONTROL) && !BI::platform::keyDown(BI::PlatformKey::LEFT_SHIFT)) {
        switch (key) {
            case GLFW_KEY_A: g_selectedInput->highlightFromToPos(0, -1); break;

            case GLFW_KEY_INSERT:
            case GLFW_KEY_C: g_selectedInput->onCopy(); break;

            case GLFW_KEY_V: g_selectedInput->onPaste(); break;

            case GLFW_KEY_X: g_selectedInput->onCut(); break;

            case GLFW_KEY_BACKSPACE:
            case GLFW_KEY_DELETE: g_selectedInput->onDelete(true, key == GLFW_KEY_DELETE); break;
    ...
    if (action == GLFW_PRESS) {
        if (!BI::platform::keyDown(BI::PlatformKey::LEFT_CONTROL)) {
            switch (key) {
                case GLFW_KEY_HOME: g_selectedInput->onHomeKey(BI::platform::keyDown(BI::PlatformKey::LEFT_SHIFT)); break;

                case GLFW_KEY_END: g_selectedInput->onEndKey(BI::platform::keyDown(BI::PlatformKey::LEFT_SHIFT)); break;

                default: break;
            }
        }

        if (BI::platform::keyDown(BI::PlatformKey::LEFT_SHIFT) && !BI::platform::keyDown(BI::PlatformKey::LEFT_CONTROL)) {
            switch (key) {
                case GLFW_KEY_INSERT: g_selectedInput->onPaste(); break;

                default: break;
            }
        }

        if (!BI::platform::keyDown(BI::PlatformKey::LEFT_SHIFT) && !BI::platform::keyDown(BI::PlatformKey::LEFT_CONTROL) && key == GLFW_KEY_ENTER) {
...
// this fixes it :D
void BetterCCEGLView::onGLFWMouseCallBack(GLFWwindow *window, int button, int action, int mods)
{
    CCEGLView::onGLFWMouseCallBack(window, button, action, mods);

    if (!g_selectedInput || button != GLFW_MOUSE_BUTTON_1 || action == 2)
        return;
...
// doesnt work. lol!
bool BetterKeyboardDispatcher::dispatchKeyboardMSG(cocos2d::enumKeyCodes key, bool isKeyDown, bool isKeyRepeat)
{
    if (!g_selectedInput)
    ...
    if (isKeyDown || isKeyRepeat) {
        if (!g_modifier_keys_state.ctrlDown && !g_modifier_keys_state.shiftDown) {
            switch (key) {
                case enumKeyCodes::KEY_Escape: g_selectedInput->deselectInput(); break;

                case enumKeyCodes::KEY_Backspace:
                case enumKeyCodes::KEY_Delete: g_selectedInput->onDelete(false, key == enumKeyCodes::KEY_Delete); break;
        ...
        switch (key) {
            case enumKeyCodes::KEY_Right: g_selectedInput->onRightArrowKey(g_modifier_keys_state.ctrlDown, g_modifier_keys_state.shiftDown); break;

            case enumKeyCodes::KEY_Left: g_selectedInput->onLeftArrowKey(g_modifier_keys_state.ctrlDown, g_modifier_keys_state.shiftDown); break;

            default: break;
        }
    }

    if (isKeyDown && g_modifier_keys_state.ctrlDown && !g_modifier_keys_state.shiftDown) {
        switch (key) {
            case enumKeyCodes::KEY_A: g_selectedInput->highlightFromToPos(0, -1); break;

            case enumKeyCodes::KEY_Insert:
            case enumKeyCodes::KEY_C: g_selectedInput->onCopy(); break;

            case enumKeyCodes::KEY_V: g_selectedInput->onPaste(); break;

            case enumKeyCodes::KEY_X: g_selectedInput->onCut(); break;

            case enumKeyCodes::KEY_Backspace:
            case enumKeyCodes::KEY_Delete: g_selectedInput->onDelete(true, key == enumKeyCodes::KEY_Delete); break;
    ...
    if (isKeyDown) {
        if (!g_modifier_keys_state.ctrlDown) {
            switch (key) {
                case enumKeyCodes::KEY_Home: g_selectedInput->onHomeKey(g_modifier_keys_state.shiftDown); break;

                case enumKeyCodes::KEY_End: g_selectedInput->onEndKey(g_modifier_keys_state.ctrlDown); break;
        ...
        if (g_modifier_keys_state.shiftDown && !g_modifier_keys_state.ctrlDown) {
            switch (key) {
                case enumKeyCodes::KEY_Insert: g_selectedInput->onPaste(); break;
...
void BetterTouchDispatcher::touches(cocos2d::CCSet *touches, cocos2d::CCEvent *event, unsigned int type)
{
    if (!g_selectedInput)
